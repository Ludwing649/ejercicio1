...
const { registrations, Registration } = require('../models/regitration.model');
...
const getAllRegistrationById = async (req, res, next) => {
    try {
        const { id } = req.params;
    ...
    const checkout = async (req, res, next) => {
        try {
            ...
            const regitration = await Registration.findOne({ where: (id) });
            if (!registration) {
                return res.status(404).json({
                    status: 'error',
                    message: 'Registration no found',
    ...
    const cancelRegitration = async (req, res, next) => {
        try {
            const { id } = req.params;
            const regitration = await Registration.findOne({ where: (id) });
